# Update GK Circle: push local code to GitHub, then git pull on NUC only.
#
# Usage:
#   .\scripts\update-project.ps1
#   .\scripts\update-project.ps1 -Message "feat: live test recovery"
#   .\scripts\update-project.ps1 -SkipCommit   # push existing commits only
#
# NUC path is FIXED: ~/apps/gkcircle (never any other directory)
...
# --- 2. NUC: git pull ONLY in ~/apps/gkcircle ---
$pullScript = @'
set -e
...
On the NUC, ensure GitHub deploy key is configured:
  bash ~/apps/gkcircle/scripts/setup-nuc-github-deploy-key.sh
  ssh -T git@github.com
  cd ~/apps/gkcircle && git pull
"@
}
