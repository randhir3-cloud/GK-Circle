...
function queryDB(sql) {
  ...
  const cmd = `docker compose exec -T db psql -U gk_circle -d gk_circle -t -A -c "${singleLineSql.replace(
    /"/g,
    '\\"'
  )}"`;
...
async function runSmokeTests() {
  console.log(
    "=== EXAM-P7-T03 Instructor Result Release Management Smoke Tests ==="
  );
  ...
  try {
    ...
    const account = {
      ...
      password: `Gkc!${randomUUID()}aA9`,
      firstName: "Test",
      lastName: "Instructor",
    };

    console.log("1. Registering a uniquely tagged instructor account...");
    await page.goto(`${WEB_BASE}/account/register`, {
    ...
    const badSchedResp = await apiCall(
      "PATCH",
      `/api/v1/quizzes/${quizId}/results/settings`,
      {
    ...
    if (badSchedResp.status !== 400) {
      throw new Error(
        `Expected 400 when SCHEDULED policy missing date, got ${badSchedResp.status} body=${badSchedResp.raw}`
      );
    }
    ...
    if (
      updateResp.data.result_release_policy !== "SCHEDULED" ||
      updateResp.data.is_currently_released !== false
    ) {
      throw new Error(
        `Settings update failed or released status incorrect: ${JSON.stringify(
          updateResp.data
        )}`
      );
    }
    ...
    const releaseResp = await apiCall(
      "POST",
      `/api/v1/quizzes/${quizId}/results/release`
    );
    console.log(` -> Status: ${releaseResp.status}`);
    if (
      !releaseResp.data.results_released ||
      !releaseResp.data.is_currently_released
    ) {
      throw new Error(
        `Manual release override failed: ${JSON.stringify(releaseResp.data)}`
      );
    }
    ...
    const repeatReleaseResp = await apiCall(
      "POST",
      `/api/v1/quizzes/${quizId}/results/release`
    );
    ...
    if (repeatReleaseResp.status !== 200) {
      throw new Error(
        `Repeated manual release failed with status ${repeatReleaseResp.status}`
      );
    }
    ...
    const immediateResp = await apiCall(
      "PATCH",
      `/api/v1/quizzes/${quizId}/results/settings`,
      {
    ...
    if (
      immediateResp.data.results_scheduled_at !== null ||
      immediateResp.data.is_currently_released !== true
    ) {
      throw new Error(
        `IMMEDIATE transition did not clear scheduled date: ${JSON.stringify(
          immediateResp.data
        )}`
      );
    }
    console.log(
      " ✓ Case 4 Passed: IMMEDIATE policy cleared schedule and set results_released=true."
    );

    // Case 6: Manual release rejected on IMMEDIATE policy
    console.log("\n[Case 4b]: Manual release rejected on IMMEDIATE policy...");
    const badReleaseResp = await apiCall(
      "POST",
      `/api/v1/quizzes/${quizId}/results/release`
    );
    ...
    if (badReleaseResp.status !== 400) {
      throw new Error(
        `Expected 400 when manually releasing IMMEDIATE policy, got ${badReleaseResp.status}`
      );
    }
    console.log(
      " ✓ Case 4b Passed: Manual release rejected on IMMEDIATE policy."
    );
    ...
    const unauthResp = await unauthPage.evaluate(
      async ({ url }) => {
    ...
    if (unauthResp !== 401 && unauthResp !== 403) {
      throw new Error(
        `Expected 401/403 for unauthorized learner update, got ${unauthResp}`
      );
    }
    console.log(
      " ✓ Case 5 Passed: Unauthenticated learner blocked with HTTP 401/403."
    );
    await unauthContext.close();

    // Case 8: Fetch release status endpoint
    console.log("\n[Case 6]: Fetching release status metadata...");
    const statusResp = await apiCall(
      "GET",
      `/api/v1/quizzes/${quizId}/results/status`
    );
    console.log(` -> Status: ${statusResp.status}`);
    if (
      !statusResp.data.quiz_id ||
      statusResp.data.total_submitted_attempts === undefined
    ) {
      throw new Error(
        `Release status response invalid: ${JSON.stringify(statusResp.data)}`
      );
    }
    console.log(
      " ✓ Case 6 Passed: Release status endpoint returned valid metadata."
    );

    // Case 9: Page refresh state persistence
    console.log("\n[Case 9]: Verifying page refresh state persistence...");
    await page.goto(`${WEB_BASE}/quiz/${quizId}/results/manage`, {
    ...
    if (!pageText.includes("Result Release Administration")) {
      throw new Error(
        "Result release management page failed to load rendered state."
      );
    }
    console.log(
      " ✓ Case 9 Passed: Management UI rendered persisted state successfully."
    );
